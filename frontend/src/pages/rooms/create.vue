      ...
      <form @submit.prevent="save">
        ...
        <div class="flex items-baseline p-4 border-b border-white-grey">
          ...
          <div class="w-2/5">
            ...
            <span class="block text-xs italic text-red" v-if="buildingIdErrors.length > 0">
              {{ buildingIdErrors[0] }}
            </span>
          </div>
        </div>
        <div class="flex items-baseline p-4 border-b border-white-grey">
          ...
          <div class="w-2/5">
            ...
            >
              ...
              <option v-for="(floor, key) in floors" :value="key">{{ floor }}</option>
            </select>
          </div>
        </div>
        <div class="flex items-baseline p-4 border-b border-white-grey">
        ...
        <div class="flex items-baseline p-4 border-b border-white-grey">
          ...
          <div class="w-2/5">
            ...
            >
              <option value="">--Choose Status--</option>
              <option v-for="(status, key) in roomStatuses" :value="key">{{ status }}</option>
            </select>
            <span class="block text-xs italic text-red" v-if="statusErrors.length > 0">
              {{ statusErrors[0] }}
            </span>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async save() {
      ...
      this.saving = true
      try {
        await this.$store.dispatch('rooms/store', this.form)
        this.$toasted.success('Room created successfully')
