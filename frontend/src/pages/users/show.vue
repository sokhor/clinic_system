  ...
  <div class="w-full">
    <div class="w-full flex flex-row items-center justify-between pt-4 pb-6">
      <h1 class="inline text-grey-darkest text-xl font-bold">
        <router-link class="text-blue hover:text-blue-light" to="/users"><i class="fas fa-arrow-left"></i></router-link> / User Details
      </h1>
      <div>
        <BaseButton sm color="primary" title="Edit user" @click="edit(user)">
          <i class="fas fa-edit"></i>
        </BaseButton>
        <BaseButton sm color="danger" title="Delete user" :waiting="user._deleting" @click="destroy(user)" class="ml-2">
